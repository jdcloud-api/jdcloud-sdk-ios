/* Copyright 2018 JDCLOUD.COM

   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License at

     http:#www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.

   Monitoring Rules APIs
   云监控规则相关接口，提供创建、查询、修改、删除监控规则等功能

   
   Contact: 

   NOTE: This class is auto generated by the jdcloud code generator program.
 */


import Foundation
import JDCloudSDKCore


///  查询报警历史
        ///         /// 检索条件组合优先级从高到低为
        ///         /// 1. serviceCode
        ///         /// 1.1 serviceCode + resourceId
        ///         /// 1.2 serviceCode + resourceIds
        ///         /// 2. serviceCodes
        ///         /// 3. 用户所有规则
public class DescribeAlarmHistoryAllRegionRequest:JdCloudRequest
{
    /// 当前所在页，默认为1
    var pageNumber:Int64?

    /// 页面大小，默认为20；取值范围[1, 100]
    var pageSize:Int64?

    /// 产品线
    var serviceCode:String?

    /// 资源Id
    var resourceId:String?

    /// 规则Id
    var alarmId:String?

    /// 正在报警, 取值为1
    var alarming:Int64?

    /// 开始时间
    var startTime:String?

    /// 结束时间
    var endTime:String?

    /// 服务码或资源Id列表
        /// filter name 为serviceCodes表示查询多个产品线的规则
        /// filter name 为resourceIds表示查询多个资源的规则
    var filters:Filter?




    enum DescribeAlarmHistoryAllRegionRequestRequestCodingKeys: String, CodingKey {
        case pageNumber
        case pageSize
        case serviceCode
        case resourceId
        case alarmId
        case alarming
        case startTime
        case endTime
        case filters
    }

    public override func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: DescribeAlarmHistoryAllRegionRequestRequestCodingKeys.self)
        try container.encode(pageNumber, forKey: .pageNumber)
        try container.encode(pageSize, forKey: .pageSize)
        try container.encode(serviceCode, forKey: .serviceCode)
        try container.encode(resourceId, forKey: .resourceId)
        try container.encode(alarmId, forKey: .alarmId)
        try container.encode(alarming, forKey: .alarming)
        try container.encode(startTime, forKey: .startTime)
        try container.encode(endTime, forKey: .endTime)
        try container.encode(filters, forKey: .filters)

    }
}
