...
///  提交转码作业请求对象
@objc(SubmitTranscodeJobRequestObject)
public class SubmitTranscodeJobRequestObject:NSObject,Codable{
    ...
    required public init(from decoder: Decoder) throws {
        ...
        {
            self.templateIds = try decoderContainer.decode([Double?]?.self, forKey: .templateIds)
        }
        if decoderContainer.contains(.watermarkIds)
        {
            self.watermarkIds = try decoderContainer.decode([Double?]?.self, forKey: .watermarkIds)
        }
    }
}
