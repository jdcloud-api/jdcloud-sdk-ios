...
public class NcJDCloudClient :JDCloudClient{
    ...
    public func modifyQuotaAsync(request:ModifyQuotaRequest,requestComplation:@escaping ExecuteResult<ModifyQuotaResult>) throws {
        ncJDCloudClient = self
        try ModifyQuotaExecutor(jdCloudClient: ncJDCloudClient).executeAsync(request: request){(statusCode,result, error,data) in
             requestComplation(statusCode,result,error,data)
        }
    } 
