...
public class GprsStatusResp:NSObject,Codable{
    ...
    required public init(from decoder: Decoder) throws {
        ...
        if decoderContainer.contains(.imsi)
        {
            self.imsi = try decoderContainer.decode(String?.self, forKey: .imsi)
        }
...
public extension GprsStatusResp{
    func encode(to encoder: Encoder) throws {
        var encoderContainer = encoder.container(keyedBy: GprsStatusRespCodingKeys.self)
         ...
         try encoderContainer.encode(imsi, forKey: .imsi)
...
public class OperationIotlinkResp:NSObject,Codable{
    /// 物联网卡iccid
    var iccid:String?
    /// 操作状态(0:操作成功;1:操作失败)
    var status:String?
    ...
    enum OperationIotlinkRespCodingKeys: String, CodingKey {
        case iccid
        case status
        case message
    }
    ...
    required public init(from decoder: Decoder) throws {
        let decoderContainer = try decoder.container(keyedBy: OperationIotlinkRespCodingKeys.self)
        if decoderContainer.contains(.iccid)
        {
            self.iccid = try decoderContainer.decode(String?.self, forKey: .iccid)
        }
...
public extension OperationIotlinkResp{
    func encode(to encoder: Encoder) throws {
        var encoderContainer = encoder.container(keyedBy: OperationIotlinkRespCodingKeys.self)
         try encoderContainer.encode(iccid, forKey: .iccid)
         try encoderContainer.encode(status, forKey: .status)
